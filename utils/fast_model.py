...
def train_model(task_id, n_layers_lst, n_heads_lst, embed_dim, batch_size, epochs, project_path):
    ...
    new_folder(project_path, "parameter_selection_models")
    path_of_data_models = os.path.join(project_path, "parameter_selection_models") #path of the folder data_models

    #create the folder for specific dataset name
    new_folder(path_of_data_models, f"{dataset_name}") #_{embed_dim}") #path of the folder dataset
        ...
        for n_layer in n_layers_lst:
            for n_head in n_heads_lst:
                try: 
                    ...
                    #experiment i folder
                    new_folder(path_of_dataset, f"experiment_{experiment_num}_{embedding_size}_{n_layer}_{n_head}")
                    path_of_experiment = os.path.join(path_of_dataset, f"experiment_{experiment_num}_{embedding_size}_{n_layer}_{n_head}") #In this folder it will be saved the model and images
